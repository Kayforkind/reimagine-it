...
| **OpenSSF Scorecard: Token-Permissions** | No workflow runs with more token power than it needs | Every workflow declares read-only top-level `permissions:`; jobs escalate individually, and `contents: write` appears only in the release job — exercised through recognized release actions (`softprops/action-gh-release`, `actions/attest-build-provenance`), which is the shape Scorecard's Token-Permissions check rewards. The npm publisher holds `id-token: write` only (for provenance); nothing else writes. |
