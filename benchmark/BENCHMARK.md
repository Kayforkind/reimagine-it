...
All **15 directions** are generated by the real CLI from **4 representative sources** (library, motion studio, farm share, meteor watch) with a fixed seed. Scores reuse the same quality bar Auto itself applies: standalone HTML, source title retained, anchor retention, focus-visible, reduced-motion, `::selection`, no placeholder copy, no external asset fetch (100 max).
...
**Output diversity:** 19.9% mean pairwise difference between directions on the same source — no two tokens produce the same page.
