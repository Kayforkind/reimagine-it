...
try {
    & $py gold/awe.py
    $code = $LASTEXITCODE
    if ($code -ne 1) { throw "Expected exit 1 for gold/awe.py, got $code" }
    ...
    foreach ($k in @('awe', 'mode', 'about', 'hero', 'stretch', 'verified')) {
        if (-not ($json.PSObject.Properties.Name -contains $k)) {
            throw "shipped.json missing key: $k"
        }
    }
    Write-Host 'PASS: gold/awe.py exits 1,1,0 and shipped.json keys verified.'
}
finally {
