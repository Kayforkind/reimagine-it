...
| Existing personal page / landing page / dashboard / plain `index.html` — user wants a redesign | **webpage** — same words, same content, held by a designed page. Load [webpage-craft.md](webpage-craft.md). If they added a second word (`artistic` / `dashboard` / `photography` / `cinematic` (aka `3d`, `webgl`) / `ecommerce` / `landing` / `portfolio`), also load the matching pack in [domains/](domains/). | `python <folder>/run.py` to screenshot before/after into `compare.png` |
