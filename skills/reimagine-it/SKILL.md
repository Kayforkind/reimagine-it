...
**SVG** → load [references/forms/svg.md](references/forms/svg.md). Marks in the field, type in the gutter. No label on a path. **Alive-micro default:** 2–4 loops on marks (weenie-breathe + one fact-loop + hover pairing). Brief `still` / `no-motion` / `print` freezes loops.

**3js** → load [references/forms/3js.md](references/forms/3js.md). Reserved HUD strip. Silhouettes from the source. Not a primitive demo. **Alive-micro default:** idle life on 2–3 meshes (weenie-turn, flow along a source curve, sun-breath) plus camera ease / wide-drift. Brief `still` pins the camera and freezes meshes.
